...
def plot_confusion_matrix(predict, ground_truth, title=None, cmap=plt.cm.Blues):
    ...
    plt.tight_layout()
    plt.savefig("result/cm_baseline_adam100.png", transparent=True)
    plt.show()
...
def write_result(paths, outputs):
    ...
    with open("result_baseline_adam100.csv", "w") as f:
        f.write("path,output\n")
...
def main():
    ...
    # モデル構成，学習した重みの保存
    model.save("keras_model/my_model_baseline_adam100.h5")
